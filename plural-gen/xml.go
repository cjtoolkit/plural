		...
		for _, relation := range strings.Split(and, "and") {
			...
			for _, rh := range strings.Split(rhs, ",") {
				if parts := strings.Split(rh, ".."); len(parts) == 2 {
					...
					if lvar == "ops.N" {
						...
						rhor = append(rhor, fmt.Sprintf("plural.IntInRange(%s %% %s, %s, %s)", lvar, lmod, from, to))
					} else {
						rhor = append(rhor, fmt.Sprintf("plural.IntInRange(%s, %s, %s)", lvar, from, to))
					}
				} else {
					rany = append(rany, rh)
				}
			...
			if len(rany) > 0 {
				...
				if lvar == "ops.N" {
					...
					rhor = append(rhor, fmt.Sprintf("plural.IntEqualsAny(%s %% %s, %s)", lvar, lmod, rh))
				} else {
					rhor = append(rhor, fmt.Sprintf("plural.IntEqualsAny(%s, %s)", lvar, rh))
				}
			}
			r := strings.Join(rhor, " || ")
			if len(rhor) > 1 {
